...
```markdown
![说明文字](assets/图.png)      ← 标准 Markdown
![[图.png]]                     ← Obsidian 嵌入语法（从剪贴板粘图就会写成这样）
![[图.png|说明文字]]             ← 带说明，会成为图片的 alt
```

从 Obsidian 粘贴的截图文件名常带空格（`屏幕截图 2026-09-18 191232.png`）。
工具会把它正确包进尖括号后再交给解析器——这是 CommonMark 的要求，
不加尖括号整段不会被识别为图片（图片会静默变成一行文字）。

图片可以放在文章目录、`assets/`、或 `assets/` 的任意子目录下，工具按此顺序查找。
找不到时报 `IMG105` ERROR 并说明该放哪里，**不会**在成品里留下一行看不懂的文字。
...
**这个仓库只放工具，不放你的文章。** 文章放在 `content_root` 指向的目录里，
可以在任意位置（本机目录、Obsidian 仓库、同步盘，甚至另一个 Git 仓库）：

```bash
inloop info      # 查看当前解析到的内容目录，以及它来自哪一级配置
inloop list      # 列出内容目录里的全部文章
```

内容目录按以下优先级解析（任务书 §3.3）：
...
`examples/2026/` 下的文章覆盖了各种写法（代码块、公式降级、表格、深浅卡片、
图片、外链），用作排版回归样本——测试会**自动遍历它们**做端到端构建。
用自己的内容做校验请用：

```bash
inloop check --all          # 检查内容目录里的全部文章
```
