...
> | 2026-09-25 | §7 新增 7.3–7.5：`list`、`delete`、`check --all`、全局 `--content`、机器可读输出 `--json` | 内容管理需要增删查；Obsidian 插件需要机器可读输出作为接口，否则只能解析给人看的彩色文字 |
> | 2026-09-26 | §3.3：兜底目录改为 `content/` 并排除出版本控制 | 原文的 `articles/` 兜底会把「作者的文章」与「工具的示例」混在同一个仓库里；现在文章默认写在 `content/`，该目录被 `.gitignore` 排除——开箱可用，但绝不入库 |
> | 2026-09-25 | §3：移除 `drafts/`；新增 `examples/` | 草稿应跟随内容目录（与文章放一起），不该留在工具仓库；示例文章原在 `articles/`，现独立为 `examples/` 以区分"工具的示例"与"你的内容" |
...
| 4 | `<工具仓库>/content` | 兜底，保证未配置时仍可运行 |

**第 4 级（兜底）在 `.gitignore` 中被排除**：文章是作者的内容，不是工具仓库的
交付物，因此默认位置虽然方便（开箱即用），但**不会进版本控制**。
正式使用建议改用第 1–3 级，把内容与工具彻底分开。
...
列出 `content_root` 下的全部文章（编号、标题、栏目、状态、日期），
并显示内容目录的绝对路径。

**为什么需要它**：内容目录有四级解析来源，"以为在读 A、实际在读 B"
是最容易犯的错。列表为空时应当先确认目录对不对，而不是怀疑文章丢了。

### 7.4 删除文章

```bash
inloop delete 007-my-article          # 默认二次确认
inloop delete 007-my-article --yes    # 跳过确认（供脚本/插件使用）
```

行为约束：

1. **破坏性操作，默认必须确认。** 确认界面先列出**将删除的文件清单**与
   图片数量，再询问。
2. **非交互环境不得静默执行。** 无法征求确认时一律取消，除非显式传 `--yes`。
3. **只能删内容目录内的文章目录。** 目标解析到内容目录本身或目录之外时拒绝执行。
4. 状态为 `published` 的文章额外提示一次。
5. 删除后提示索引可能过期（不自动重建索引，避免隐式写入）。

**为什么需要确认**：内容目录通常不在 Git 里（可能由坚果云之类的同步盘管理），
删掉的文章没有提交历史可回滚。

### 7.5 全局选项与机器可读输出

**`--content <路径>`**（全局，放在子命令之前）：临时指定内容目录，
优先级高于环境变量与配置。

**`--json`**：让命令输出机器可读的 JSON，供编辑器插件等外部程序调用。

约定：

- 适用范围：`list`、`check`、`check --all`、`build-wechat`
- **stdout 只含 JSON**，人类可读的进度与提示走 stderr；不得夹带 ANSI 颜色码
- JSON 顶层是对象；命令失败时以非 0 退出码结束，并仍在 stdout 给出结构化错误
- 字段名一旦发布即视为稳定契约，改动等同于破坏性变更

**为什么需要它**：Obsidian 插件等外部界面必须拿到结构化数据。
解析给人看的彩色文本极其脆弱——改一句提示文案就会让插件失效。
